...
void simulation::update_world_shapes() {
  ...
  parallel_for_range(objects_.begin(), objects_.end(), [&](std::shared_ptr<object> const& object) {
    auto const world_shape(
        std::make_pair(object, object->getShape().transform(object->position(), object->orientation())));
...
void simulation::update_bounding_boxes() {
  ...
  parallel_for_range(objects_.begin(), objects_.end(), [&](object_t const& object) {
    auto const bounding_box(std::make_pair(object, world_shapes_[object].bounding_box()));
...
void simulation::update_quadtree() {
  ...
  for(auto const& object : objects_) {
    quadtree_.insert(std::make_pair(object, bounding_boxes_[object]));
  }
}
...
void simulation::find_islands() {
  ...
  std::for_each(
      collisions_.begin(), collisions_.end(), [&](std::pair<object_t, std::unordered_set<object_t>> const& collision) {
...
void simulation::find_contacts() {
  ...
  std::size_t index(0);
  std::for_each(
      islands_.begin(), islands_.end(), [&](std::shared_ptr<std::set<std::pair<object_t, object_t>>> const& island) {
        std::vector<std::pair<object_t, object_t>> const collision_list(island->begin(), island->end());

        parallel_for_range(
            collision_list.begin(), collision_list.end(), [&, index](std::pair<object_t, object_t> const& collision) {
              ...
              if(a_shape.intersects(b_shape)) {
                ...
                std::tuple<bool, Vector2f, float, Vector2f, Vector2f> const distance_data(b_core.distance(a_core));
                ...
                if(parallel(a_segment.getVector(), b_segment.getVector())) {
                  contact const contact(a, b, a_segment.middle(), b_segment.middle(), normal);
                  ...
                  if(a_core.corner(ap)) {
                    ap += (ap - a->position()).normalized() * 2.0;
                    bp = b_feature.closest(ap);
                  } else if(b_core.corner(bp)) {
                    bp += (bp - b->position()).normalized() * 2.0;
                    ap = a_feature.closest(bp);
                  }

                  contact const contact(a, b, ap, bp, normal);
                  if(contact.relative_velocity() >= 0.0) {
                    std::lock_guard<std::mutex> lock(contacts_mutex_);
                    contacts_[index].emplace_back(contact);
                  }
...
void simulation::step(float const delta_time, float const time_step) {
  time_ += delta_time;
  ...
  while(accumulator_ >= time_step) {
    for(auto object : objects_) {
      if(!object->kinematic() && !object->frozen()) {
        object->force() = Vector2f(0.0, 9.81);
        object->torque() = 0.0;
      }
    }
...
void simulation::resolve_collisions() {
  std::for_each(contacts_.begin(), contacts_.end(), [&](std::vector<contact> const& island) {
    parallel_for_range(island.begin(), island.end(), [&](contact const& contact) {
      ...
      float const restitution(std::max(a->getMaterial().restitution(), b->getMaterial().restitution()));

      float impulse_numerator, impulse_denominator, impulse;
      if(a->kinematic()) {
        auto const brv(b->linear_velocity() + cross(br, b->angular_velocity()));

        impulse_numerator = (brv * -(1.0 + restitution)).dot(normal);
        impulse_denominator = 1.0 / b->mass() + (cross(br, normal) * cross(br, normal)) / b->moment_of_inertia();
        impulse = impulse_numerator / impulse_denominator;

        b->linear_velocity() += normal * (impulse / b->mass());
        b->angular_velocity() += cross(br, normal * impulse) / b->moment_of_inertia();
      } else if(b->kinematic()) {
        auto const arv(a->linear_velocity() + cross(ar, a->angular_velocity()));

        impulse_numerator = (arv * -(1.0 + restitution)).dot(normal);
        impulse_denominator = 1.0 / a->mass() + (cross(ar, normal) * cross(ar, normal)) / a->moment_of_inertia();
        impulse = impulse_numerator / impulse_denominator;

        a->linear_velocity() += normal * (impulse / a->mass());
        a->angular_velocity() += cross(ar, normal * impulse) / a->moment_of_inertia();
      } else {
        Vector2f const vab(a->linear_velocity() + cross(ar, a->angular_velocity()) - b->linear_velocity() -
                           cross(br, b->angular_velocity()));

        impulse_numerator = (vab * -(1.0 + restitution)).dot(normal);
        impulse_denominator = 1.0 / a->mass() + 1.0 / b->mass() +
                              (cross(ar, normal) * cross(ar, normal)) / a->moment_of_inertia() +
                              (cross(br, normal) * cross(br, normal)) / b->moment_of_inertia();
        impulse = impulse_numerator / impulse_denominator;
  ...
  std::for_each(contacts_.begin(), contacts_.end(), [&](std::vector<contact> const& island) {
    ...
    for(unsigned int i(0); i < n; ++i) {
      ...
      parallel_for_range_index(island.begin(), island.end(), [&](contact const& contact_j, std::size_t const j) {
        ...
        if(i_a == j_a) {
          A(i, j) +=
              i_normal.dot(j_normal / i_a->mass() + cross(i_ar, cross(j_ar, j_normal)) / i_a->moment_of_inertia());
        }
        if(i_a == j_b) {
          A(i, j) -=
              i_normal.dot(j_normal / i_a->mass() + cross(i_ar, cross(j_br, j_normal)) / i_a->moment_of_inertia());
        }
        if(!i_b->kinematic() && i_b == j_a) {
          A(i, j) -=
              i_normal.dot(j_normal / i_b->mass() + cross(i_br, cross(j_ar, j_normal)) / i_b->moment_of_inertia());
        }
        if(!i_b->kinematic() && i_b == j_b) {
          A(i, j) +=
              i_normal.dot(j_normal / i_b->mass() + cross(i_br, cross(j_br, j_normal)) / i_b->moment_of_inertia());
        }
      });
    }
    ...
    parallel_for_range_index(island.begin(), island.end(), [&](contact const& contact, std::size_t const i) {
      ...
      if(!b->kinematic()) {
        auto const arv(a->linear_velocity() + cross(ar, a->angular_velocity()));
        auto const brv(b->linear_velocity() + cross(br, b->angular_velocity()));
        B(i) += 2.0 * cross(normal, b->angular_velocity()).dot(arv - brv);
      }

      B(i) += normal.dot(a->force() / a->mass() + cross(ar, a->torque() / a->moment_of_inertia()) +
                         cross(cross(ar, a->angular_velocity()), a->angular_velocity()));
      B(i) -= normal.dot(b->force() / b->mass() + cross(br, b->torque() / b->moment_of_inertia()) +
                         cross(cross(br, b->angular_velocity()), b->angular_velocity()));
    });
    ...
    parallel_for_range_index(island.begin(), island.end(), [&](contact const& contact, std::size_t const i) {
      ...
      if(!a->kinematic()) {
        ...
        a->torque() += cross(ar, normal * force) / a->moment_of_inertia();
      }
      if(!b->kinematic()) {
        auto const br(contact.bp() - b->position());
        b->force() -= normal * (force / b->mass());
        b->torque() -= cross(br, normal * force) / b->moment_of_inertia();
      }
    });
  });
}

std::tuple<Vector2f, Vector2f, float, float> simulation::evaluate(
    std::shared_ptr<object> const& initial,
    float const time,
    float const time_step,
    std::tuple<Vector2f, Vector2f, float, float> const& derivative) const {
  return std::make_tuple(initial->linear_velocity() + std::get<1>(derivative) * time_step,
                         initial->force(),
                         initial->angular_velocity() + std::get<3>(derivative) * time_step,
                         initial->torque());
}

void simulation::integrate(float const time_step) {
  parallel_for_range(objects_.begin(), objects_.end(), [&](object_t const& object) {
    if(!object->kinematic()) {
      auto const a(evaluate(object, time_, 0.0, std::tuple<Vector2f, Vector2f, float, float>()));
      ...
      object->position() +=
          (std::get<0>(a) + (std::get<0>(b) + std::get<0>(c)) * 2.0 + std::get<0>(d)) * (1.0 / 6.0) * time_step;
      object->linear_velocity() +=
          (std::get<1>(a) + (std::get<1>(b) + std::get<1>(c)) * 2.0 + std::get<1>(d)) * (1.0 / 6.0) * time_step;
      object->orientation() +=
          (std::get<2>(a) + (std::get<2>(b) + std::get<2>(c)) * 2.0 + std::get<2>(d)) * (1.0 / 6.0) * time_step;
      object->angular_velocity() +=
          (std::get<3>(a) + (std::get<3>(b) + std::get<3>(c)) * 2.0 + std::get<3>(d)) * (1.0 / 6.0) * time_step;

      auto const movement_threshold(0.01);
      if(object->linear_velocity().norm() <= movement_threshold &&
         std::abs(object->angular_velocity()) <= movement_threshold) {
        object->frozen(true);
        object->linear_velocity() = Vector2f();
