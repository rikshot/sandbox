...
namespace sandbox {
  ...
  std::set<std::pair<boost::shared_ptr<object>, boost::shared_ptr<object>>> simulation::find_collisions() {
    ...
    unsigned const max_tasks(objects_.size() <= threads_ ? objects_.size() : threads_);
    unsigned int const objects_range(objects_.size() <= threads_ ? 1 : objects_.size() / threads_);
    for(unsigned int i(0), last(i + 1 == max_tasks); i < max_tasks; ++i, last = i + 1 == max_tasks) {
      tasks.push_back(std::async([&, i, last]() {
        auto & start(bounding_boxes.begin() + (objects_range * i));
        auto & end(last ? bounding_boxes.end() : start + objects_range);
        for(auto & j(start); j != end; ++j) {
          auto const & object(j->first);
          if(!object->kinematic()) {
            auto const & bounding_box(j->second);
  ...
  std::vector<contact> simulation::find_contacts(std::set<std::pair<boost::shared_ptr<object>, boost::shared_ptr<object>>> const & collisions) const {
    ...
    std::vector<std::future<void>> tasks;
    unsigned const max_tasks(collision_list.size() <= threads_ ? collision_list.size() : threads_);
    unsigned int const collision_list_range(collision_list.size() <= threads_ ? 1 : collision_list.size() / threads_);
    for(unsigned int i(0), last(i + 1 == max_tasks); i < max_tasks; ++i, last = i + 1 == max_tasks) {
      tasks.push_back(std::async([&, i, last]() {
        auto & start(collision_list.begin() + (collision_list_range * i));
        auto & end(last ? collision_list.end() : start + collision_list_range);
        for(auto & j(start); j != end; ++j) {
          auto const a(j->first);
  ...
  void simulation::resolve_collisions(std::vector<contact> const & contacts) {
    ...
    std::vector<std::future<void>> tasks;
    unsigned const max_tasks(contacts.size() <= threads_ ? contacts.size() : threads_);
    unsigned int const contacts_range(contacts.size() <= threads_ ? 1 : contacts.size() / threads_);
    for(unsigned int i(0), last(i + 1 == max_tasks); i < max_tasks; ++i, last = i + 1 == max_tasks) {
      tasks.push_back(std::async([&, i, last]() {
        auto & start(contacts.begin() + (contacts_range * i));
        auto & end(last ? contacts.end() : start + contacts_range);
        for(auto & j(start); j != end; ++j) {
          auto const & contact(*j);
  ...
  void simulation::integrate(double const time_step) {
    ...
    for(unsigned int i(0), last(i + 1 == max_tasks); i < max_tasks; ++i, last = i + 1 == max_tasks) {
      tasks.push_back(std::async([&, i, last]() {
        auto & start(objects_.begin() + (objects_range * i));
        auto & end(last ? objects_.end() : start + objects_range);
        for(auto & j(start); j != end; ++j) {
          auto & object = *j;
          if(!object->kinematic()) {
