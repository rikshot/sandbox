...
namespace sandbox {
  ...
  public:
      ...
      simulation(float const width, float const height) : width_(width), height_(height), time_(0.0), accumulator_(0.0), quadtree_(rectangle(Vector2f(0.0, 0.0), Vector2f(width_, height_))) {
      }
    ...
    private:
      ...
      std::tuple<Vector2f, Vector2f, float, float> evaluate(object_t const & initial, float const time, float const time_step, std::tuple<Vector2f, Vector2f, float, float> const & derivative) const;
